    ...
    >
      ...
      <div class="flex flex-col items-center">
        <span
          ...
          :class="STATUS[phase.status].color"
        >{{ STATUS[phase.status].icon }}</span>
        <div
      ...
      <div class="pb-6 min-w-0">
        <div class="mb-1 flex flex-wrap items-center gap-2">
          <span class="text-xs font-mono text-muted">{{ phase.version }}</span>
          ...
          <span
            ...
            :class="{
              ...
              'bg-surface text-muted border border-border': phase.status === 'planned',
            }"
          >{{ STATUS[phase.status].label }}</span>
        </div>
        <p class="text-xs text-muted mb-2">{{ phase.description }}</p>
        <ul v-if="phase.highlights.length" class="space-y-0.5">
          <li
            v-for="h in phase.highlights"
            :key="h"
